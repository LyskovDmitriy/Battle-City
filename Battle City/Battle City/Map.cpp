...
{
	MAPS.insert(make_pair("Особая карта", "Maps/specialMap.txt"));
...
void Map::fill()
{
	for (int y = 4;y < sizeY;y += 5)
	{
		int x = rand() % sizeX / 2;
		int lengthX = sizeX / 4 + rand() % (sizeX / 2);
		int lengthY = 2;
		addLine(y, x, lengthX, lengthY);
	}
	for (int x = 0;x < sizeX;x += 5)
	{
		int y = rand() % sizeY / 2;
		int lengthY = sizeY / 4 + rand() % (sizeY / 2);
		int lengthX = 2;
		addLine(y, x, lengthX, lengthY);
	}
}
...
void Map::addLine(int x, int y, int lengthX, int lengthY)
{
	for (int i = y;i < y + lengthY && i < sizeY;i++)
	{
		for (int j = x;j < x + lengthX && j < sizeX;j++)
		{
			map[i][j] = ENVIRONMENT_CH;
		}
	}
}
